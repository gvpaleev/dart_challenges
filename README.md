...
Реализуйте функцию «countdown», которая принимает аргумент «int n» и печатает все числа от «n» до 0 с задержкой в одну секунду между каждым числом.

Затем вызовите «countdown(5)» внутри метода «main()» и убедитесь, что он работает должным образом.

Программа должна напечатать «Готово» после завершения обратного отсчета.
